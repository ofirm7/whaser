...
export class AnthropicWorkflowLlm implements WorkflowLlm {
  ...
  async improveChat({ systemPrompt, messages, executeToolCall }: { systemPrompt: string; messages: WorkflowRuntimeMessage[]; executeToolCall: (name: string, input: Record<string, unknown>) => Promise<string> }): Promise<{ text: string; usage: { inputTokens: number; outputTokens: number } }> {
    const apiTools = [
      ...
      { name: 'copy_my_style', description: "Make THIS agent write in the OWNER's own personal style/voice. Studies the owner's own WhatsApp messages and tunes the agent to match how they actually write. Call this when the owner asks to make the agent talk/sound/write like them. No input needed.", input_schema: { type: 'object', additionalProperties: false, properties: {} } },
    ];
